...
const ROLE_KEYWORDS: { [key: string]: string[] } = {
  ...
  "HVAC / Thermal Engineer": [
    ...
    "Ventilation", "Fire Safety Codes", "ASHREAE Standards",
          ...
          <motion.div variants={itemVariants}>
            ...
            <Tabs defaultValue="shortlisted" className="w-full">
              <TabsList className="grid w-full grid-cols-2 bg-secondary/30 border border-primary/50 shadow-neon-glow-sm">
                <TabsTrigger 
                  value="shortlisted" 
                  className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-neon-glow data-[state=active]:border-primary data-[state=active]:border-b-0 data-[state=active]:skew-x-[-10deg] dataYou've found another error, and I sincerely apologize. That was a careless copy-paste mistake on my part. I accidentally left a comment from my notes inside the code, which caused the syntax error. Thank you for catching that.

I am now fixing the `ROLE_KEYWORDS` list in `src/pages/Index.tsx` to remove the invalid entry and correct a small typo I noticed. This will resolve the build error.

<dyad-write path="src/pages/Index.tsx" description="Fixing a syntax error in the keyword list and correcting a typo.">
import React, { useState } from "react";
import ResumeUploadForm from "@/components/ResumeUploadForm";
import CandidateList from "@/components/CandidateList";
import { Candidate } from "@/types";
